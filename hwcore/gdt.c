...
#define BUILD_GDTE(descr_privilege_level, is_code)                                                                                                                                                     \
  ((struct x86_segment_descriptor){                                                                                                                                                                    \
      ...
      .segment_type = ((is_code) ? 0xb : 0x3), /* With descriptor_type (below) = 1 (code/data),                                                                                                        \
...
sos_ret_t sos_gdt_setup(void) {
  ...
  asm volatile("lgdt %0          \n\
                 ...
                 movw %%ax,  %%gs"
               :
               : "m"(gdtr), "i"(SOS_BUILD_SEGMENT_REG_VALUE(0, FALSE, SOS_SEG_KCODE)), "i"(SOS_BUILD_SEGMENT_REG_VALUE(0, FALSE, SOS_SEG_KDATA))
               : "memory", "eax");
