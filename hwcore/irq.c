...
sos_irq_handler_t sos_irq_handler_array[SOS_IRQ_NUM] = {
    NULL,
};

sos_ret_t sos_irq_setup(void) { return sos_i8259_setup(); }
...
sos_ret_t sos_irq_set_routine(int irq_level, sos_irq_handler_t routine) {
  ...
  /* If the irq is to be enabled, update the IDT with the IRQ
     wrapper */
  if (routine != NULL) {
    retval = sos_idt_set_wrapper(SOS_IRQ_BASE + irq_level, (sos_vaddr_t)sos_irq_wrapper_array[irq_level], 0 /* CPL0 routine */);
    /* A problem occured */
    if (retval != SOS_OK)
      sos_irq_handler_array[irq_level] = NULL;
  } else /* Disable this idt entry */
  {
    retval = sos_idt_set_wrapper(SOS_IRQ_BASE + irq_level, (sos_vaddr_t)NULL /* Disable IDTE */, 0 /* Don't care */);
  }
