...
sos_ret_t sos_idt_setup() {
  ...
  for (i = 0; i < SOS_IDTE_NUM; i++) {
    ...
    idte->reserved = 0;
    idte->flags = 0;
    idte->type = 0x6; /* Interrupt gate (110b) */ // p5 & p6 article, avant l'appel le processeur va donc désactiver les interruptions
...
sos_ret_t sos_idt_set_wrapper(int index, sos_vaddr_t wrapper_address, int lowest_priviledge /* 0..3 */) {
  ...
  idte = idt + index;
  if (wrapper_address != (sos_vaddr_t)NULL) {
    idte->offset_low = wrapper_address & 0xffff;
    idte->offset_high = (wrapper_address >> 16) & 0xffff;
    idte->dpl = lowest_priviledge;
    idte->present = 1; /* Yes, there is a handler */
  } else {
    idte->offset_low = 0; /* Disable this IDT entry */
    idte->offset_high = 0;
    idte->dpl = 0;
